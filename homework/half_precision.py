...
class HalfLinear(torch.nn.Linear):
    def __init__(
        ...
        """
        # Initialize the parent class with float32 (default)
        ...
        # Disable gradients since we won't backpropagate through this layer
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Convert output back to original dtype (float32)
...
class HalfBigNet(torch.nn.Module):
    ...
    class Block(torch.nn.Module):
        def __init__(self, channels: int):
            super().__init__()
            # Use HalfLinear instead of torch.nn.Linear for half-precision weights
    ...
    def __init__(self):
        super().__init__()
        # Use LayerNorm in full precision to avoid numerical instability
        # Use HalfLinear (via Block) for weights in half precision
        self.model = torch.nn.Sequential(
